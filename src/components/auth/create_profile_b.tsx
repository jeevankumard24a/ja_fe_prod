...
// 1) Reusable envelope aligned with backend
type ApiEnvelope<T> = {
...
// 2) Domain payloads live under `data`
interface UserIdCheckPayload {
    isUnique: boolean;
}
...
// 3) API callers: pass inner shape as generic T
// NOTE: we log at start/end and forward x-action-id if present
const Check_User_Name_Uniqueness = async (v_user_id: string) => {
    ...
    const res = await make_api_request<UserIdCheckPayload>(`/api/auth/register/check-userid-unique/`, {
        method: "POST",
        headers: {
...
export default function Create_Profile() {
    ...
    // Log mount/unmount (debug = browser-only)
    useEffect(() => {
        log.debug("Create_Profile.mount");
        return () => log.debug("Create_Profile.unmount");
    }, []);

    // When the userid input is valid enough to trigger query, let’s note it
    useEffect(() => {
    ...
    const mutation = useMutation<RegisterResponse, ApiError, Register_Type>({
        ...
        onSuccess: (res, vars) => {
            ...
            if (res.data?.success) {
                ...
                reset();
                router.push(
                    `/login?message=${encodeURIComponent("Login-after-Register-Password-Sent-to-Your-Email")}`
                );
                toast.success("Registration successful! Please login.", { className: "font-kalam" });
            } else {
                ...
                toast.error(res.data?.message || "Registration failed.", { className: "font-kalam" });
            }
        },
        onError: (err) => {
            const actionId = getActionId();
            const msg = `${err.message}${err.requestId ? ` (ref: ${err.requestId})` : ""}`;
    ...
    const onSubmit = async (data: Register_Type) => {
        ...
        try {
            ...
            if (isError_cun) {
                const msg =
                    error_cun instanceof ApiError
                        ? `${error_cun.message}${error_cun.requestId ? ` (ref: ${error_cun.requestId})` : ""}`
                        : "Error checking User ID. Please try again or contact support.";
                log.error("register.userid.check.error", {
                    actionId,
                    requestId: error_cun instanceof ApiError ? error_cun.requestId : undefined,
            ...
            if (isUnique === true) {
                ...
                toast.error("Please check your network or contact support.", { className: "font-kalam" });
                return;
            }
            ...
            log.error("register.flow.error", {
                ...
                ...(err instanceof ApiError ? { requestId: err.requestId, code: (err as any).code } : {}),
            });
            const msg =
    ...
    useEffect(()=>{
       log.info("Hellooooooooooooooooooo  Godddddddddddddddd ,  Please make my Project Success, if not please take me to you.");
    },[])
