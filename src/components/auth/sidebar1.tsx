...
export default function NavChrome({ baseImageUrl, baseDomainUrl }: NavChromeProps) {
    const [sidebarOpen, setSidebarOpen] = useState(false);
    const pathname = usePathname();

    const navItems: NavItem[] = [
        { label: "Colleges", href: "/colleges", icon: FaUniversity },
        { label: "Job Exams", href: "/job-exams", icon: BiSolidSelectMultiple },
        { label: "Coaching Centers", href: "/coaching-centers", icon: PiExamFill },
        { label: "Training Centers", href: "/training-centers", icon: FaCode },
        { label: "Schools", href: "/schools", icon: IoSchoolSharp },
        { label: "Companies", href: "/companies", icon: SiBmcsoftware },
        { label: "Contact Us", href: "/contact-us", icon: GrContact },
    ...
    return (
        <>
            ...
            <Dialog
                open={sidebarOpen}
                onClose={() => setSidebarOpen(false)}  // important: actually close
                        ...
                        <div className="flex items-center grow flex-col gap-y-5 overflow-y-auto rounded-l-[40px] bg-gradient-to-r1 px-6 pb-4">
                            ...
                            <nav className="flex flex-1 flex-col gap-y-4">
                                <div className="flex-auto rounded-3xl text-sm/6">
                                    <div className="grid grid-cols-1 gap-4 sm:gap-6 md:gap-8 p-4 font-ibmm font-medium">
                                        {navItems.map((item) => {
                          ...
                          `}
                                                    ...
                                                    title={item.label}
                                                >
                                                    <Icon className={`text-xl ${active ? "text-white" : "text-white"}`} />
                                                    <span className="text-sm font-medium font-ibmm whitespace-nowrap">
                            {item.label}
                          </span>
                                                </Link>
                                            );
                ...
                <div className="flex w-full grow flex-col gap-y-5 overflow-y-auto rounded-l-[40px] bg-gradient-to-r1 px-6 pb-4">
                    ...
                    <nav className="flex flex-1 w-full flex-col items-center">
                        {sidebarItems.map((item, i) => (
                            <div key={item.label} className={`${i > 0 ? "pt-6" : ""} flex flex-col items-center`}>
                                <Link
                                    href={item.href}
            ...
            {/* Top Navigation Bar (padded to avoid sidebar overlap) */}
            <div className="lg:pl-[160px] sticky top-0 z-40 w-full h-[100px] border-b border-gray-300 bg-white flex items-center px-4">
                {/* Mobile Menu Button */}
