...
import CourseSyllabus from "@/components/dashboard/course_syllabus";
...
export default function Course_Home({ course_id }: { course_id: string }) {
  console.log("CourseIddddddddddddd22222222222222", course_id);
  ...
  const {
    ...
    queryKey: ["user_id", course_id],
    queryFn: () => load_dashboard(course_id),
    enabled: true,
    retry: false,
  });
  ...
  return (
    <div className="flex flex-col justify-start c-text-c1 mt-1 items-center w-full">
      ...
      {/* Tabs Section */}
      <div className="w-11/12 max-11/12 mt-14">
        <TabGroup>
          ...
          <TabPanels>
            <TabPanel>
              <Suspense fallback={<div>Loading Posts...</div>}>
                <CourseSyllabus course_id={course_id} userId={"abc"} />
              </Suspense>
            </TabPanel>
            <TabPanel>
