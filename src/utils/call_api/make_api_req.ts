...
export async function make_api_request<T = unknown>(
  ...
  const method = String((init as any)?.method || "").toUpperCase();
  if (res.status === 204 || res.status === 205 || res.status === 304 || method === "HEAD") {
    return {
      status: "success",
      error: false,
  ...
  if (raw && isJson) {
    try {
      ...
      throw new ApiError(
          ...
          "Invalid server JSON",
          "INVALID_JSON",
          { rawPreview: raw.slice(0, PREVIEW_LIMIT), contentType },
          rid,
          aid
      );
    }
  ...
  if (!res.ok) {
    ...
    throw new ApiError(
        ...
        code,
        body ?? {
          rawPreview: (raw ?? "").slice(0, PREVIEW_LIMIT),
          contentType,
