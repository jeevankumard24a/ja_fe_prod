...
const devConsoleFormat = combine(
    ...
    printf(({ level, message, timestamp, stack, ...meta }) => {
        ...
        return stack ? `[${timestamp}] ${level}: ${message}\n${stack}${rest}` : `[${timestamp}] ${level}: ${message}${rest}`;
    })
);
...
declare global {
    ...
    var __NEXT_LOGGER__: Logger | undefined;
}
...
function createBaseLogger(): Logger {
    ...
    // ---- keep Winston internals; wrap level methods; child returns real child ----
    const wrapped: Logger & { child(meta: Record<string, unknown>): Logger } = {
        ...base,
        ...
        child(meta: Record<string, unknown>) {
            const c = base.child(meta) as Logger;
            return {
                ...c,
                info: (...a: unknown[]) => (c as any).log(asEntry('info', a)),
                warn: (...a: unknown[]) => (c as any).log(asEntry('warn', a)),
                error: (...a: unknown[]) => (c as any).log(asEntry('error', a)),
                debug: (...a: unknown[]) => (c as any).log(asEntry('debug', a)),
            } as Logger;
        },
    } as any;
...
/* ---------------- singleton export ---------------- */
export const logger: Logger = global.__NEXT_LOGGER__ ?? createBaseLogger();
if (process.env.NODE_ENV !== 'production') global.__NEXT_LOGGER__ = logger;
